...
if (enabled) {
	...
	var m_x = mouse_x - x, m_y = mouse_y - y;
	var ca = dcos(image_angle), sa = dsin(image_angle);
	if (image_angle != 0) {var t_x = m_x, t_y = m_y; m_x =  t_x * ca - t_y * sa; m_y = t_x * sa + t_y * ca;}
	if (image_xscale == 0) m_x = 0; else m_x /= image_xscale;
	if (image_yscale == 0) m_y = 0; else m_y /= image_yscale;
	m_x -= origin_x; m_y -= origin_y;
}
    ...
    if (multiline) {
        if (action != 3) {
            while (true) {
                ...
                var i = 1, t = abs(cursor_position);
				while (t > new_line_list[| i]) i++; 
					cursor_line = i - 1;
